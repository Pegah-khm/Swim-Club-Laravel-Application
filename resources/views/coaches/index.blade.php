    ...
    <main>
        <div class="mx-auto max-w-screen-xl py-6 sm:px-6 lg:px-8">
                ...
                <table class="w-full max-w-screen-lg text-center border border-gray-300">
                    ...
                    @foreach ($coaches as $member)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('coaches.show', $member->id) }}" class="action-button">View
                                    details</a>

                                @if(auth()->user()->role === 'club_official')
                                    <a href="{{ route('coaches.edit', $member->id) }}" class="action-button">Edit</a>

                                    <form action="{{ route('coaches.destroy', $member->id) }}" method="POST"
            ...
            <div class="mt-1 mx-23 space-x-4">
                ...
                @if(auth()->user()->role === 'club_official')
                    <a href="{{ route('coaches.create') }}" class="button">Add a new coach</a>
                @endif
            </div>
        </div>
        </div>
